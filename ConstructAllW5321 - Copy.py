...
try:
	config = open("configW5321pre.txt", "r")
	for line in config:
	...
	print("'configW5321pre.txt' not found or has errors")
...
eur_gbp = 0.8952
P_gas = (((constant_gas_value / SPs) / 29.30711) * 10) # 37.57
...
gas_price_full = [((((price / 29.30711) * 10) / efficiency) * plant_output) for price in gas_price[1:]]
...
df['I'] = np.where(((df['D'] > ((start_time / 24)) * 48) & (df['D'] <= (((start_time / 24) + (operating_hours / 24)) * 48))), df['H'], "")
df['Z'] = np.where(df['G'] <= best_charge_trigger, df['G'], "")
df['AA'] = np.where((df['H'] >= best_discharge_trigger) & (df['D'] > ((start_time / 24) * 48)) & (df['D'] <= (((start_time / 24) + (operating_hours / 24)) * 48)), df['H'], "")

# Convert df['Z'] to numeric
df['Z'] = pd.to_numeric(df['Z'], errors='coerce')

df['AB'] = np.where(df['Z'].isna(), "", 
                    np.where(df['AI'].shift(-1) < 0, (stored_energy_max - stored_energy_min)*df['Z'], 
                             np.where(df['AI'].shift(-1) == (stored_energy_max - stored_energy_min), 
                                      np.where((df['D'] > ((start_time / 24) * 48)) & (df['D'] < (((start_time / 24) + (operating_hours / 24)) * 48)), (stored_energy_max * ((heat_loss / 100) / (daily_time_steps / (time_step_hours))))*df['Z'], ((((stored_energy_max * ((heat_loss / 100) / (daily_time_steps / (time_step_hours)))))+((plant_output * boost)*(time_step/3600)))*df['Z'])), 
                                      df['Z']*((stored_energy_max - stored_energy_min)-df['AI'].shift(-1) + ((stored_energy_max * ((heat_loss / 100) / (daily_time_steps / (time_step_hours)))))))))

 # Convert df['AA'] and df['AI'] to numeric
df['AA'] = pd.to_numeric(df['AA'], errors='coerce')
df['AI'] = pd.to_numeric(df['AI'], errors='coerce')
       
df['AC'] = np.where((df['AA'].shift() > 0) & (df['AI'].shift() > 0), df['G'] * ((plant_output * boost) / (3600 / time_step)), np.nan)
df['AF'] = df['G'] * ((plant_output * boost) * 0.5)

# Convert df['AB'] to numeric
df['AB'] = pd.to_numeric(df['AB'], errors='coerce')

df['AG'] = df['AB'].expanding().apply(lambda x: x[::-1].idxmax() if np.any(x) else np.nan)
df['AH'] = np.where((df['AB'] != 0) & (df['AI'].shift() > 0), (((stored_energy_max - stored_energy_min) - df['AI'].shift() + ((stored_energy_max * ((heat_loss / 100) / (daily_time_steps / (time_step_hours))))) + ((plant_output * boost) / (3600 / time_step))) * df['G']), 0) + np.where((df['AB'] != 0) & (df['AI'].shift() < 0), (((stored_energy_max - stored_energy_min)) * df['G']), 0) + np.where((df['AB'] != "") & (df['AI'].shift() == ((stored_energy_max - stored_energy_min))), df['AB'], 0)
df['AI'] = np.where(df['AB'] == "", df['AI'].shift() - ((stored_energy_max * ((heat_loss / 100) / (24 / (time_step_hours)))) + np.where((df['D'] > ((start_time / 24) * 48)) & (df['D'] <= (((start_time / 24) + (operating_hours / 24)) * 48)), (((plant_output * boost) / efficiency) * (time_step / 3600)) * (df['AG'] / df['AG']), 0)), ((stored_energy_max - stored_energy_min)))
df['AJ'] = np.where((df['AI'] < -1) & (df['D'] > ((start_time / 24) * 48)) & (df['D'] <= (((start_time / 24) + (operating_hours / 24)) * 48)), df['G'], 0) + np.where((df['AI'] < 0) & (df['AI'] > -1) & (df['D'] > ((start_time / 24) * 48)) & (df['D'] <= (((start_time / 24) + (operating_hours / 24)) * 48)), (-(df['AI']) / (df['AI'].shift() - df['AI'])) * ((plant_output * boost) / 2) * df['G'], 0)
df['AK'] = np.where((df['AI'] < -1) & (df['D'] > ((start_time / 24) * 48)) & (df['D'] <= (((start_time / 24) + (operating_hours / 24)) * 48)), core_mass, 0) + np.where((df['AI'] < 0) & (df['AI'] > -1) & (df['D'] > ((start_time / 24) * 48)) & (df['D'] <= (((start_time / 24) + (operating_hours / 24)) * 48)), (-(df['AI']) / (df['AI'].shift() - df['AI'])) * ((plant_output * boost) / 2) * core_mass, 0)
df['AM'] = (fuel_co2 * 1000) / (efficiency * efficacy)
df['AN'] = np.where((df['D'] > ((start_time / 24) * 48)) & (df['D'] <= (((start_time / 24) + (operating_hours / 24)) * 48)), ((df['AL'] * (plant_output * boost) / 2) / 1000), 0)
df['AO'] = np.where((df['D'] > ((start_time / 24) * 48)) & (df['D'] <= (((start_time / 24) + (operating_hours / 24)) * 48)), ((df['AM'] * (plant_output * boost) / 2) / 1000), 0)
df['AP'] = np.where(df['AI'] < -1, df['AN'], 0) + np.where((df['AI'].shift() > 0) & (df['AI'] < 0), (df['AI'] / (df['AI'] - df['AI'].shift())) * df['AN'], 0)
df['AQ'] = np.where(df['AI'] < -1, df['AO'], 0) + np.where((df['AI'].shift() > 0) & (df['AI'] < 0), (df['AI'] / (df['AI'] - df['AI'].shift())) * df['AO'], 0)        

spent_access = df['AB'].iat[-2]
constant_spent_access = df['AH'].iat[-2]
earned_access = df['AC'].iat[-2]
constant_earned_access = df['AF'].iat[-2]
profit_access = df['AD'].iat[-1]
profit_access_constant = df['AK'].iat[-1]
spent_avg_access = df['AB'].iat[-1]
electrical_cost_access = df['G'].iat[-3]
SPs_access = df['G'].iat[-2]
empty_access = df['AI'].iat[-3]
discharged_access = df['AL'].iat[-2]
total_required_access = df['AL'].iat[-1]
electrical_carbon_intensity_access = df['AN'].iat[-2]
gas_carbon_intensity_access = df['AO'].iat[-2]
electrical_store_carbon_intensity_access = df['AP'].iat[-2]
gas_store_carbon_intensity_access = df['AQ'].iat[-2]

df.loc[spent_access, 'AB'] = df['AB'].iloc[1:-2].sum()
df.loc[constant_spent_access, 'AH'] = df['AH'].iloc[1:-2].sum()
df.loc[earned_access, 'AC'] = df['AC'].iloc[1:-2].sum()
df.loc[constant_earned_access, 'AF'] = df['AF'].iloc[1:-2].sum()
df.loc[profit_access, 'AD'] = df['AC'].iloc[-2] - df['AB'].iloc[-2]
df.loc[profit_access_constant, 'AK'] = df['AF'].iloc[-2] - df['AI'].iloc[-2]
df.loc[spent_avg_access, 'AB'] = df['AB'].iloc[1:-2].mean()
df.loc[electrical_cost_access, 'G'] = (df['G'].iloc[1:-2].sum() * df['AD'].iloc[4]) / 2
df.loc[SPs_access, 'G'] = df['G'].iloc[1:-2].count()
df.loc[empty_access, 'AI'] = (df['AI'].iloc[1:-2] <= 0).sum()
df.loc[discharged_access, 'AL'] = df['AL'].iloc[1:-2].sum()
df.loc[total_required_access, 'AL'] = ((df['AL'].iloc[1:-2].count() * df['AD'].iloc[10] * df['AD'].iloc[15]) * (df['AD'].iloc[13] / 1000) * (df['AD'].iloc[11] - df['AD'].iloc[9])) / 3600
df.loc[electrical_carbon_intensity_access, 'AN'] = df['AN'].iloc[1:-2].sum()
df.loc[gas_carbon_intensity_access, 'AO'] = df['AO'].iloc[1:-2].sum()
df.loc[electrical_store_carbon_intensity_access, 'AP'] = df['AP'].iloc[1:-2].sum()
df.loc[gas_store_carbon_intensity_access, 'AQ'] = df['AQ'].iloc[1:-2].sum()
